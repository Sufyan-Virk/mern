...
export const createCar = async (req, res, next) => {
  ...
  let insertedCar = await Car.findOne({
    $or: [{ registrationNo }, { category, make, model }],
  });
  if (insertedCar) {
  ...
  res
    ...
    .json(new HTTPResponse(car, 'Car added successfully'));
};
...
export const updateCar = async (req, res, next) => {
  ...
  let updatedcar = await Car.findByIdAndUpdate(
    req.params.id,
    { model, color, make, registrationNo, category },
    { new: true },
  );
  res
    .status(StatusCodes.OK)
    .send(new HTTPResponse(updatedcar, 'Car updated successfully'));
};
...
export const deleteCar = async (req, res, next) => {
  ...
  res
    .status(StatusCodes.OK)
    .send(new HTTPResponse(deleteCar, 'Car deleted successfully'));
};
