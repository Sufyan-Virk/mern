...
export const signup = async(req, res, next) => {
  ...
  await user.encryptPassword(password)
   await user.save();
  res.status(StatusCodes.CREATED).json(new HTTPResponse(user, 'Signed up successfully'))
}
...
export const login = async(req, res, next) => {
  ...
  if(await existedUser.comparePassword(password)) {
   ...
   res.status(StatusCodes.OK).json(new HTTPResponse({...responseUser, token}, 'Logged in successfully'))
  }
  else{
    return next(new HTTPError('Wrong Email/Password', StatusCodes.UNAUTHORIZED))
  }
